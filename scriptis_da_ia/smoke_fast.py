...
def main() -> int:
    ...
    test_ponder_fingerprint_determinismo_e_sensibilidade()
    test_ponder_package_action_e_deterministico_mesmo_estado_mesma_decisao()
    test_ponder_match_dedicada_nao_compartilha_com_match_principal_em_concorrencia()
    print()
...
def _import_server_module():
    """
    Importa BOT/engine_server/server.py -- import deferido (só chamado de
    dentro dos testes de pondering) porque server.py mora numa árvore de
    diretórios separada de scriptis_da_ia/ e faz efeitos colaterais de
    import (loga sessão em arquivo). FastAPI/uvicorn/pydantic já são
    dependências deste projeto (requirements.txt), então o import é seguro
    no mesmo ambiente que roda este smoke suite.
    """
    server_dir = (Path(__file__).resolve().parent.parent / "BOT" / "engine_server")
...
def test_ponder_fingerprint_determinismo_e_sensibilidade() -> None:
    # Pondering (bloco 478): o fingerprint decide se um resultado
    # ADIANTADO (calculado durante o turno do OPONENTE) ainda vale pra
    # decisão real -- por isso precisa ser (a) determinístico (mesma
    # entrada sempre dá o mesmo hash) e (b) sensível a QUALQUER mutação
    # relevante isolada (falha SEMPRE fechada: qualquer dúvida sobre se o
    # estado mudou tem que invalidar o resultado adiantado, nunca reusar
    # por engano).
    import copy

    bot = {"hand": [{"code": "OP01-001", "cost": 1, "power": 1000, "powerAtk": 1000,
                     "rested": False, "justPlayed": False, "deckUniqueId": 10,
                     "donAttached": 0, "actionUsed": False, "cantAttack": False}],
           "board": [], "life": [], "leader": None, "stage": None, "trash": [],
           "deckCount": 40, "activeDon": 3, "restedDon": 0}
    opp = {"hand": [], "board": [], "life": [], "leader": None, "stage": None,
           "trash": [], "deckCount": 40, "activeDon": 0, "restedDon": 0}
    mem = {"opp_hand": [], "opp_life": [], "own_life": [], "opp_deck": []}

    fp1 = sim_bridge.ponder_fingerprint(bot, opp, mem)
    fp2 = sim_bridge.ponder_fingerprint(bot, opp, mem)
    check("ponder_fingerprint e deterministico (mesma entrada, mesmo hash 2x)",
          fp1 == fp2)

    bot_rested = copy.deepcopy(bot)
    bot_rested["hand"][0]["rested"] = True
    check("ponder_fingerprint muda quando 'rested' de uma carta muda",
          sim_bridge.ponder_fingerprint(bot_rested, opp, mem) != fp1)

    bot_don = copy.deepcopy(bot)
    bot_don["activeDon"] = 4
    check("ponder_fingerprint muda quando activeDon (bot) muda",
          sim_bridge.ponder_fingerprint(bot_don, opp, mem) != fp1)

    opp_deck = copy.deepcopy(opp)
    opp_deck["deckCount"] = 39
    check("ponder_fingerprint muda quando deckCount do oponente muda",
          sim_bridge.ponder_fingerprint(bot, opp_deck, mem) != fp1)

    opp_life = copy.deepcopy(opp)
    opp_life["life"] = [{"code": "OP01-002", "cost": 0, "power": 0, "powerAtk": 0,
                         "rested": False, "justPlayed": False, "deckUniqueId": 99,
                         "donAttached": 0, "actionUsed": False, "cantAttack": False}]
    check("ponder_fingerprint muda quando a vida do oponente muda",
          sim_bridge.ponder_fingerprint(bot, opp_life, mem) != fp1)

    mem_reveal = copy.deepcopy(mem)
    mem_reveal["opp_hand"] = [123]
    check("ponder_fingerprint muda quando o snapshot de reveals (MatchMemory) muda",
          sim_bridge.ponder_fingerprint(bot, opp, mem_reveal) != fp1)


def _build_ponder_test_state(power_atk: int = 8000, power_def: int = 4000):
    """Cenario sintetico com 1 unico candidato real (atacar o lider do
    oponente) -- sem ambiguidade de Monte Carlo, decisao deterministica
    mesmo sem fixar seed: nao ha personagem no board do oponente (sem
    alvo-character), a mao do bot esta vazia (sem candidato 'play'), e o
    poder do atacante ja mata o lider sem precisar de DON."""
    atacante = mk("TST-PONDER-ATK", "Atacante Teste Pondering",
                  power=power_atk, cost=4, color="Red")
    atacante._deck_uid = 100
    me = GameState(leader=mk("TST-PONDER-LB", "Lider Bot Teste", power=5000,
                             cost=0, card_type="LEADER", color="Red"),
                   don_available=5, turn=3)
    me.field_chars = [atacante]
    me.hand = []
    opp = GameState(leader=mk("TST-PONDER-LO", "Lider Oponente Teste",
                              power=power_def, cost=0, card_type="LEADER",
                              color="Blue"), turn=3)
    opp.field_chars = []
    opp.hand = []
    return me, opp
...
def test_ponder_package_action_e_deterministico_mesmo_estado_mesma_decisao() -> None:
    # Propriedade de correcao MAIS IMPORTANTE do pondering (bloco 478):
    # pondering muda QUANDO a decisao e calculada (durante o turno do
    # oponente, adiantado), NUNCA o que e decidido. Roda a MESMA busca
    # (sim_bridge.choose_action) duas vezes pro MESMO estado sintetico
    # (imitando "caminho real" vs "caminho de pondering") e prova que o
    # payload empacotado por server._package_action e byte-identico nas
    # duas vezes.
    server = _import_server_module()

    match1 = OPTCGMatch((mk("TST-PONDER-LB", "L", card_type="LEADER"), []),
                        (mk("TST-PONDER-LO", "L", card_type="LEADER"), []))
    match2 = OPTCGMatch((mk("TST-PONDER-LB", "L", card_type="LEADER"), []),
                        (mk("TST-PONDER-LO", "L", card_type="LEADER"), []))

    me1, opp1 = _build_ponder_test_state()
    trace1: dict = {}
    action1 = sim_bridge.choose_action(
        me1, opp1, match1, timeout=3.0,
        allowed_types={"play", "attack", "attach_don", "activate"},
        trace_out=trace1)
    payload1, reason1 = server._package_action(action1, me1, opp1, sim_bridge, match1, trace1)

    me2, opp2 = _build_ponder_test_state()
    trace2: dict = {}
    action2 = sim_bridge.choose_action(
        me2, opp2, match2, timeout=3.0,
        allowed_types={"play", "attack", "attach_don", "activate"},
        trace_out=trace2)
    payload2, reason2 = server._package_action(action2, me2, opp2, sim_bridge, match2, trace2)

    check("pondering: mesmo estado produz a MESMA acao escolhida nas duas vezes",
          reason1 == reason2 == "acao escolhida")
    check("pondering: payload empacotado e byte-identico entre 'caminho real' e 'caminho de pondering'",
          payload1 == payload2)
    check("pondering: a acao escolhida e mesmo o ataque ao lider (cenario sem ambiguidade)",
          payload1["type"] == "attack" and payload1["targetId"] == 0)


def test_ponder_match_dedicada_nao_compartilha_com_match_principal_em_concorrencia() -> None:
    # Pondering (bloco 478): a razao pra usar uma OPTCGMatch DEDICADA pro
    # job de pondering (nunca a instancia compartilhada usada pela decisao
    # real) e que OPTCGMatch._simulate_sequence_values usa
    # self._suppress_replay_log como flag mutavel de INSTANCIA durante a
    # busca -- duas threads chamando metodos de busca na MESMA instancia
    # ao mesmo tempo seria uma corrida real (confirmado por leitura direta
    # do codigo antes de implementar, nao so suposto). Este teste roda
    # choose_action em DUAS instancias SEPARADAS, concorrentemente, em
    # estados DIFERENTES, e confirma que nenhuma quebra nem vaza estado.
    import threading

    match_real   = OPTCGMatch((mk("TST-PONDER-LB", "L", card_type="LEADER"), []),
                              (mk("TST-PONDER-LO", "L", card_type="LEADER"), []))
    match_ponder = OPTCGMatch((mk("TST-PONDER-LB", "L", card_type="LEADER"), []),
                              (mk("TST-PONDER-LO", "L", card_type="LEADER"), []))

    resultados: dict = {}
    erros: list = []

    def rodar(nome, match, power_atk, power_def):
        try:
            me, opp = _build_ponder_test_state(power_atk, power_def)
            trace: dict = {}
            action = sim_bridge.choose_action(
                me, opp, match, timeout=3.0,
                allowed_types={"play", "attack", "attach_don", "activate"},
                trace_out=trace)
            resultados[nome] = action
        except Exception as e:
            erros.append((nome, e))

    t1 = threading.Thread(target=rodar, args=("real", match_real, 8000, 4000))
    t2 = threading.Thread(target=rodar, args=("ponder", match_ponder, 9000, 3000))
    t1.start()
    t2.start()
    t1.join(timeout=10)
    t2.join(timeout=10)

    check("pondering: 2 OPTCGMatch separadas rodam concorrentemente sem excecao",
          not erros)
    check("pondering: as duas threads completaram e devolveram uma acao",
          "real" in resultados and "ponder" in resultados
          and resultados["real"] is not None and resultados["ponder"] is not None)
    check("pondering: as duas instancias voltam pro estado de repouso (_suppress_replay_log=False), sem vazamento entre threads",
          match_real._suppress_replay_log is False and match_ponder._suppress_replay_log is False)
